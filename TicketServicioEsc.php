...
// Obtener datos del servicio de ESCANER - CORREGIDO
$sql = "SELECT s.Id_Servicio, s.nombreServicio, s.Descripcion, s.User_Id,
               e.TipoDocumento, e.PrecioTotal, e.MontoPago,
               e.DiaEntrega, e.MesEntrega, e.AnioEntrega, u.Usuario
        FROM servicio s
...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(0, 10, utf8_decode('Ticket de Servicio de Escaneo'), 0, 1, 'C');
        $this->Ln(10);
    }
...
// Detalles del servicio de ESCANEO
$pdf->SetFont('Arial', 'B', 12);
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(40, 10, utf8_decode('Tipo de documento:'), 0, 0);
$pdf->SetFont('Arial', '', 12);
$pdf->Cell(0, 10, utf8_decode($servicio['TipoDocumento']), 0, 1);

$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(40, 10, utf8_decode('Fecha de entrega:'), 0, 0);
$pdf->SetFont('Arial', '', 12);
...
// Salida del PDF
$pdf->Output('I', 'Ticket_Escaneo_' . $servicio['Id_Servicio'] . '.pdf');
?>